...
* `user_managed_identity` - (Optional) The user managed identity to attach to the AKS cluster (string)
* `managed_identity` - (Optional) Is Managed Identity enabled for the AKS cluster? (bool)
* `private_dns_zone` - (Optional/Computed) Private DNS Zone for the cluster node pools (string)
